# あなたのふるさと納税AI - ふるさと納税パーソナライズ推薦サービス MVP
...
17. **Google AdSense広告**: 全記事ページにAuto Ads広告を配置
   - 導入文の後と関連記事セクションの前の2箇所に配置
   - AdBannerコンポーネントで一元管理
...
### v1.17.0 (2025-01-24)
- **Google AdSense統合**: 全21記事ページにAuto Ads広告を配置
  - `AdBanner`コンポーネントを新規作成（`src/components/AdBanner.tsx`）
  - 各記事に2箇所配置：導入文の後、関連記事セクションの前
  - AdSenseクライアントID: `ca-pub-5781326713622626`
  - サイト所有権確認のため`<head>`にスクリプトを配置
  - 審査開始済み（承認後に広告が自動表示される）
- **Firestoreセキュリティルール更新**: `dashboardAccess`コレクションの書き込み許可を追加
  - ダッシュボードアクセスログの記録が可能に
  - 認証済みユーザーのみ自分のログを作成可能

### v1.16.0 (2025-12-18)
- **サービス名変更**: 「ふるそな」から「あなたのふるさと納税AI」にリブランディング
  - ロゴ画像を刷新（`your-hurusato-logo.png`に変更）
  - ヘッダーロゴの横幅を拡大（240px → 320px）
  - 全ページ・全記事のサービス名を更新
- **ヒーローセクション改善**: LPとInfoページに背景画像を追加
  - 多層オーバーレイ（30%黒 + グラデーション）で文字視認性を向上
  - テキストにドロップシャドウを適用し、カラフルな背景でも読みやすく
- **SEOメッセージング最適化**: 「診断」から「提案/サポート」へ表現を変更
  - メタデータ更新：タイトル、ディスクリプション、キーワード
  - CTAボタン：「AIおすすめ診断」→「AIで返礼品を探す」
  - サービスの本質（AIによる提案・サポート）をより明確に表現
  - OpenGraph、Twitter Cardメタデータも同様に更新
- **文言調整**:
  - 「もう、12月に慌てない。」「※ クレジットカード登録不要」を削除
  - より簡潔で明確なメッセージングに統一
