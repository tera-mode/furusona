...
3. **季節性対応**: AIが現在の月を考慮して旬の商品を優先的に推薦
4. **無限スクロール**: 下にスクロールすると自動的に新しいおすすめ返礼品を表示
5. **寄付履歴管理**: 今年の寄付を一覧で管理し、残りの限度額をリアルタイム表示
6. **Firestore永続キャッシュ**: 楽天APIから取得した商品データを検索条件ベースでキャッシュ（7日間有効）
7. **レスポンシブデザイン**: モバイル・タブレット・デスクトップに最適化されたUI
8. **管理者デバッグ画面**: アルゴリズム設定・変更履歴・フロー図を可視化（`/debug`）
